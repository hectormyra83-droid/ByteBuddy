...
import type { Message, DietaryPlanRequest } from '../src/types';
...
async function answerHealthQuestion(history: Message[]): Promise<string> {
...
async function generateChatTitle(history: Message[]): Promise<string> {
...
async function generateDietaryPlan(request: DietaryPlanRequest): Promise<string> {
...
async function analyzeLoggedFood(food: string): Promise<string> {
    const prompt = `
...
export default async function handler(req: Request) {
    ...
    try {
        ...
        switch (task) {
            case 'answerHealthQuestion':
                result = await answerHealthQuestion(payload.history);
                break;
            case 'generateChatTitle':
                result = await generateChatTitle(payload.history);
                break;
            case 'generateDietaryPlan':
                result = await generateDietaryPlan(payload.request);
                break;
            case 'analyzeLoggedFood':
                result = await analyzeLoggedFood(payload.food);
                break;
            default:
                return new Response(JSON.stringify({ error: `Invalid task: ${task}` }), { status: 400, headers: { 'Content-Type': 'application/json' } });
        }
        
        return new Response(JSON.stringify({ result }), { status: 200, headers: { 'Content-Type': 'application/json' } });

    } catch (error: any) {
        console.error('Error in GenAI proxy:', error);
